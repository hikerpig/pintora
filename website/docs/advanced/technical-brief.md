...
Pintora's built-in diagrams use [nearley.js](http://nearley.js.org/) to generate context-independent syntax parsers that are easy to use, based on an improved Earley algorithm, and have decent performance (though perhaps the slowest of the mainstream solutions, but perfectly adequate for small text diagram DSLs). Diagram authors can choose between an efficient parser generator solution such as jison / PEG.js, or a handwritten parser.
