...
component ->
    "component" __ %WORD %SPACE:* %NL {%
      function(d) {
        const name = tv(d[2])
        return { type: 'component', name,  }
      }
    %}
  | "component" __ %WORD __ %L_SQ_BRACKET elementLabel:+ %R_SQ_BRACKET %SPACE:? %NL {%
      function(d) {
        const name = tv(d[2])
  ...
  #     }
  #   %}
  | shortComponent %SPACE:? %NL {% id %}
  | shortComponent __ "as" __ %WORD %SPACE:* %NL {%
      function(d) {
        const comp = d[0]
        const name = tv(d[4])
...
interface ->
    ("interface" | "()") __ (textInsideQuote | %WORD) %SPACE:* %NL {%
      function(d) {
        const _l = d[2][0]
    ...
    %}
  | ("interface" | "()") __ (textInsideQuote | %WORD) %SPACE:* "as" __ %WORD %SPACE:* %NL {%
      function(d) {
        const _l = d[2][0]
...
relationship ->
    elementReference _ relationLine _ elementReference (__ ":" __ (%WORD | %SPACE):+):? %NL {%
      function(d) {
