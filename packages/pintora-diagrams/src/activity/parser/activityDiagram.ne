...
conditionSentence ->
    "if" %SPACE:+ wordsInParens %SPACE:+ "then" (%SPACE:+ wordsInParens):? %SPACE:* %NL line:* elseClause:? _ "endif" _ %NL {%
      function(d) {
...
elseClause ->
    %SPACE:* "else" __ wordsInParens:? %SPACE:* %NL line:* {%
      function(d) {
...
whileSentence ->
    "while" __ wordsInParens (%SPACE:+ "is" __ wordsInParens):? _ %NL line:* %SPACE:* "endwhile" (%SPACE:+ wordsInParens):? %NL {%
      function(d) {
...
switchSentence ->
    "switch" __ wordsInParens %SPACE:* %NL (%SPACE:* caseClause):* %SPACE:* "endswitch" %SPACE:* %NL {%
      function(d) {
        const message = d[2]
...
caseClause ->
    "case" __ wordsInParens %SPACE:* %NL line:* {%
      function(d) {
...
forkSentence ->
    "fork" %SPACE:* %NL (__ statement):+ (_ forkAgainClause):* _ ("endfork"|"endmerge") %NL {%
      function(d) {
...
forkAgainClause ->
    "forkagain" %SPACE:* %NL (__ statement):+ {%
      function(d) {
...
group ->
    groupType __ (color %SPACE):? (%QUOTED_WORD | %VALID_TEXT) __ %L_BRACKET (__ statement):+ %SPACE:* %R_BRACKET %SPACE:* %NL {%
      function(d) {
...
noteStatement ->
	  ("note"|%START_NOTE) %SPACE:* placement %COLON words %NL {%
      function(d) {
        const text = d[4].trim()
    ...
    %}
	| ("note"|%START_NOTE) %SPACE:* placement %SPACE:* %NL multilineNoteText %NL {%
      function(d) {
